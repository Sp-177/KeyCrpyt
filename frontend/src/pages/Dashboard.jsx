...
const WelcomeCard = lazy(() => import("../components/ui/WelcomeCard"));
const FloatingHint = lazy(() => import("../components/ui/FloatingHints"));
const PasswordVault = lazy(() => import("../components/ui/PasswordVault"));
const Alert = lazy(() => import("../components/ui/Alert"));
...
export default function Dashboard() {
  ...
  if (loading || !user) {
    return <Loading isDark={isDark} />; // Full-screen loading before auth state is known
  }

  if (error) {
    return (
      <div className="w-full h-screen flex items-center justify-center text-rose-500 text-lg font-medium">
        Error: {error.message}
      </div>
    );
  }
